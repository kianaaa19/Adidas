...
with tab_channels:
    ...
    with col_a:
        ...
        fig_ch = go.Figure()
        for i, met in enumerate(["Revenue", "Profit"]):
            fig_ch.add_trace(go.Bar(
                name=met, x=ch_df["Sales Method"], y=ch_df[met],
                marker_color=["#0f62fe","#009d9a","#8a3ffc"][0] if i==0 else "#393939",
                marker_color_=CARBON_COLORS[i],
                opacity=0.9 if i==0 else 0.6,
            ))
        fig_ch.update_traces(marker_color=None)
        fig_ch2 = px.bar(
